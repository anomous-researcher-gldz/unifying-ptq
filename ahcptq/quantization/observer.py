...
class BlockMSEObserver(ObserverBase):
    """
    Blockwise MSE-based observer with grid search for min/max.
    Produces per-block qparams of shape [nbh, nbw, 1, 1] (no leading singleton).
    """
    def __init__(self, bit=8, symmetric=False, ch_axis=-1, block_size=1024):
        super().__init__(bit=bit, symmetric=symmetric, ch_axis=ch_axis)
        # print("blockmseinit")
        self.block_size = block_size 
        self.p = 2.4
        self.num = 100  # candidate count in grid search
        self.one_side_dist = None  # 'pos', 'neg', 'no'
        self._eps = 1e-8

    @torch.no_grad()
    def lp_loss(self, pred: torch.Tensor, tgt: torch.Tensor, p: float = 2.0):
        """
        Per-block Lp loss (mean over block pixels).
        Works for 2D [H,W], 3D [D,H,W], or 4D [B,C,H,W].
        Output shape: [nbh, nbw, 1, 1] (averaged over any leading dims).
        """
        x = (pred - tgt).abs().pow(p)
        return x.mean() 
    def loss_fx(self, x: torch.Tensor, new_min: torch.Tensor, new_max: torch.Tensor):
        """
        Compute lp-loss between x and per-block quantized(x) with (new_min,new_max).
        """
        scale, zero_point = self.calculate_qparams(new_min, new_max)
        # print("loss fx scale, zp shapes", scale.shape, zero_point.shape)
        x_q = fake_quantize_per_block_affine(x, scale, zero_point, self.quant_min, self.quant_max, 
        self.block_size)

        return self.lp_loss(x_q, x, p=self.p)
    
    # ---- grid searches ----
    @torch.no_grad()
    def perform_2D_search(self, x: torch.Tensor):
        """
        Enumerate per-block ranges and zp to minimize lp-loss.
        Returns best_min/best_max each [nbh, nbw, 1, 1].
        """
        blocks,meta = flatten_into_blocks(x,self.block_size)
        # blocks,meta = flatten_into_channels(x)
        x_min, x_max = compute_block_min_max(blocks)
        # print(x_min.shape,x_max.shape)
        x_max = torch.max(x_max, torch.zeros_like(x_max))  # make upper bound ≥ 0
        x_min = torch.min(x_min, torch.zeros_like(x_min))  # make lower bound ≤ 0
        # print(x_min.shape,x_max.shape)
        xrange = x_max - x_min
    ...
    @torch.no_grad()
    def perform_1D_search(self, x: torch.Tensor):
        """
        Symmetric-style per-block search (or one-sided if one_side_dist is 'pos'/'neg').
        Returns [nbh, nbw, 1, 1] tensors.
        """
        blocks,meta = flatten_into_blocks(x,self.block_size)
        # blocks,meta = flatten_into_channels(x)
        x_min, x_max = compute_block_min_max(blocks)
    ...
    def forward(self, x_orig: torch.Tensor):
        ...
        # keep device; cast to float32 for stability
        x = x_orig.detach().to(x_orig.device, torch.float32)

        # global one-side flag (match MSEObserver behavior)
        ...
        print("block mse min/max shapes", best_min.shape,best_max.shape)
