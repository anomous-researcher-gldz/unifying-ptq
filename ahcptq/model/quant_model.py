...
class QuantDecoderOurAttentionBlock(QuantizedBlock):
    def __init__(self, org_module: DecoderAttention, w_qconfig, a_qconfig, ahcptq_config, ptq4sam_config, qoutput=True, qinput=False):
        ...
        if getattr(ptq4sam_config, "ANCHOR_AWARE", False):
            ...
            anchor_a_config['num_anchors'] = ptq4sam_config.num_anchors
        else:
        ...
        self.prompt_bank = PromptAnchorBank(
            ...
            ema_momentum=0.9, #change to 0.6, was 0.9
            normalize=True,
        )
    ...
    def forward(self, qkv: tuple) -> Tensor:
        ...
        # dir_path = Path("/home/AHCPTQ/q_distributions_yolox_samb")
        # num_files = sum(1 for p in dir_path.iterdir() if p.is_file())
        # # # descriptor: [B, C]
        desc = self.prompt_bank.compute_descriptor(prompt_tokens)  # [B, C]

        # if num_files < 31:
        #     name = "/home/AHCPTQ/q_distributions_yolox_samb/qlayer_description_"+ str(torch.mean(desc))+ ".pt"
        #     torch.save(q,name)

        # observer_enabled => calibration => update anchor
        ...
        # hist = torch.bincount(anchor_ids.flatten().cpu(), minlength=3)
        ...
        # q = self.q_post_act_fake_quantize(q_proj)
