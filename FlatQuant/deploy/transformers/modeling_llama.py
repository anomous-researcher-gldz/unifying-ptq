...
try:
    ...
    _flash_attention_forward = None  # Use self._flash_attention_forward (transformers < 4.42)
...
class FlatQuantLlamaAttention(FlatQuantFP16LlamaAttention):
    ...
    def __init__(self, options, *args, **kwargs):
        ...
        _dbaf = getattr(options, 'dbaf_alpha', None)
        self.quantizer_q = deploy.nn.Quantizer(lac=self.isFlatQ, dbaf_alpha=_dbaf)
        self.quantizer_k = deploy.nn.Quantizer(lac=self.isFlatQ, dbaf_alpha=_dbaf)
        self.quantizer_v = deploy.nn.Quantizer(lac=self.isFlatQ, dbaf_alpha=_dbaf)
        ...
        self.o_proj = torch.nn.Sequential(
            deploy.nn.Quantizer(lac=self.isFlatQ, dbaf_alpha=_dbaf),
...
class FlatQuantLlamaMLP(LlamaMLP):
    def __init__(self, options, *args, **kwargs):
        ...
        self.options = options
        _dbaf = getattr(options, 'dbaf_alpha', None)
        self.quantizer_g = deploy.nn.Quantizer(lac=self.isFlatQ, dbaf_alpha=_dbaf)
        self.quantizer_u = deploy.nn.Quantizer(lac=self.isFlatQ, dbaf_alpha=_dbaf)
