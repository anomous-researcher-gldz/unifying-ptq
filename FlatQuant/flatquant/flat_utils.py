...
def save_flat_matrices(args, model, rank=None):
    ...
    # Save PCSA state (PromptBank + AnchorAwareActivationQuantizer)
    _save_pcsa_state(args, model)


def _save_pcsa_state(args, model):
    """Save PromptBank anchors and per-anchor quantizer state for each layer."""
    pcsa_state = {}
    for i in range(len(model.model.layers)):
        attn = model.model.layers[i].self_attn
        if not hasattr(attn, 'prompt_bank'):
            continue
        layer_state = {
            'prompt_bank.anchors': attn.prompt_bank.anchors.detach().cpu(),
            'prompt_bank.counts': attn.prompt_bank.counts.detach().cpu(),
        }
        q_act = attn.q_proj.act_quantizer
        if hasattr(q_act, 'anchor_scale'):
            layer_state['q_act.anchor_scale'] = q_act.anchor_scale.detach().cpu()
            layer_state['q_act.anchor_zero'] = q_act.anchor_zero.detach().cpu()
            layer_state['q_act._anchor_initialized'] = q_act._anchor_initialized.detach().cpu()
        pcsa_state[i] = layer_state
    if pcsa_state:
        pcsa_path = os.path.join(args.exp_dir, "pcsa_state.pth")
        torch.save(pcsa_state, pcsa_path)
        logging.info("saved PCSA state at {}".format(pcsa_path))
...
def load_flat_matrices(args, model, path=None):
    ...
    # Load PCSA state if available
    _load_pcsa_state(args, model, path=path)
    return model


def _load_pcsa_state(args, model, path=None):
    """Load PromptBank anchors and per-anchor quantizer state for each layer."""
    base_dir = path if path is not None else args.exp_dir
    pcsa_path = os.path.join(base_dir, "pcsa_state.pth")
    if not os.path.exists(pcsa_path):
        return
    pcsa_state = torch.load(pcsa_path)
    for i, layer_state in pcsa_state.items():
        attn = model.model.layers[i].self_attn
        if not hasattr(attn, 'prompt_bank'):
            continue
        attn.prompt_bank.anchors.copy_(layer_state['prompt_bank.anchors'])
        attn.prompt_bank.counts.copy_(layer_state['prompt_bank.counts'])
        q_act = attn.q_proj.act_quantizer
        if hasattr(q_act, 'anchor_scale') and 'q_act.anchor_scale' in layer_state:
            q_act.anchor_scale.data.copy_(layer_state['q_act.anchor_scale'])
            q_act.anchor_zero.copy_(layer_state['q_act.anchor_zero'])
            q_act._anchor_initialized.copy_(layer_state['q_act._anchor_initialized'])
    logging.info("loaded PCSA state from {}".format(pcsa_path))
