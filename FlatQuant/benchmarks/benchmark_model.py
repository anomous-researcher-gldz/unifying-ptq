...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--dbaf_alpha', type=float, default=None,
        help='DBAF fold alpha (None=disabled, 0.99=default). For W2 ablation.',
    )
    parser.add_argument(
        '--model', type=str, default=None,
        help='Single model path to benchmark (overrides model_configs list).',
    )

    args = parser.parse_args()
    if args.model is not None:
        model_configs.clear()
        model_configs.append(args.model)
