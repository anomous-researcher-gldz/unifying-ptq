...
try:
    from ahcptq.quantization.fake_quant import unfold_outliers
...
class Linear4bit(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        dbaf_tag = x.dbaf_tag
        dbaf_T = x.dbaf_T
        dbaf_alpha = x.dbaf_alpha
        x, scales_x = x.quantized_x, x.scales_x
        x = deploy.matmul(x, self.weight)
        if self.bias is not None:
            result = deploy.sym_dequant(x, scales_x, self.weight_scales) + self.bias
        else:
            result = deploy.sym_dequant(x, scales_x, self.weight_scales)
        # DBAF: unfold outliers after dequantization
        if dbaf_tag is not None and _DBAF_AVAILABLE:
            result = unfold_outliers(result, dbaf_tag, dbaf_T, dbaf_alpha)
        return result
